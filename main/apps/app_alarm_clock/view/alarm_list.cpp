...
constexpr uint32_t _button_color        = LACQUER;
constexpr uint32_t _label_color         = SILK;
constexpr uint32_t _switch_off_color    = BRONZE_D;
constexpr uint32_t _switch_on_color     = MALACHITE;
constexpr uint32_t _dialog_bg_color     = LACQUER;
constexpr uint32_t _dialog_border_color = BRONZE_D;
constexpr uint32_t _dialog_delete_color = CINNABAR;
constexpr uint32_t _dialog_cancel_color = BRONZE_D;
...
void AlarmListView::createAlarmRow(int y, int alarmId, const model::AlarmClock::Alarm& alarm)
{
    ...
    lv_obj_set_style_bg_color(row->enabledSwitch->get(), lv_color_hex(SILK), LV_PART_KNOB);
